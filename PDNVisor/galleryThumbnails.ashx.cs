    ...
    public class GalleryThumbnails : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (doc != null)
            {
                ...
                string thumbPath = Global.GetCacheDirectory() + "\\" + HttpUtility.UrlDecode(fname) + ".thumb";
                ...
                gen_thumb:
                Helper.GetImage(doc, out var filename, out byte[] contentori);
                MemoryStream ms = new MemoryStream(contentori);

                GdPicture14.DocumentFormat documentFormat = GdPicture14.DocumentFormat.DocumentFormatUNKNOWN;
                int thumbnailId = 0;
                int pageCount = 0;
                GdPictureStatus status = GdPictureDocumentUtilities.GetDocumentPreview((Stream)ms, filename, THUMBNAIL_WIDTH, THUMBNAIL_HEIGHT, THUMBNAIL_BACKGROUND_COLOR.ToArgb(), ref documentFormat, ref thumbnailId, ref pageCount);
