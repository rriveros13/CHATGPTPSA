...
{
    public static class ReferenciasPersonales
    {
        public static List<INT_CR_REF_PERSONALES> CreateReferenciasPersonales(SolicitudPersona SolPersonaOri, Session SessionIT)
        {
            List<INT_CR_REF_PERSONALES> lista = new List<INT_CR_REF_PERSONALES>();

            foreach (var item in SolPersonaOri.ReferenciasPersonales)
            {
                ...
                referencia.REL_PERSONA = item.Parentezco != null ? item.Parentezco.Nombre: string.Empty;
                referencia.TELEFONOS = item.Telefono;
                referencia.TIP_REFERENCIA = "P";
